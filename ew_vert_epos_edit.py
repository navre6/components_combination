...
for x in os.listdir(folder):
    if "InU" in x:
        inu_zip = os.path.join(folder, x)
        final_dir = os.path.splitext(inu_zip)[0]  # crea una cartella con lo stesso nome del file zip, senza estensione

        with zipfile.ZipFile(inu_zip, 'r') as zipper:
            print("Unzipping InU files...")
            zipper.extractall(final_dir)
            

        inu_data = os.listdir(final_dir)
        for y in inu_data:
            if ".tif" in y:
                y = final_dir + "\\" + y
                data_input.append(y)
...
for y in os.listdir(folder):
    if "Cos" in y:
        cos_zip = os.path.join(folder, y)
        final_dir_cos = os.path.splitext(cos_zip)[0]  # crea una cartella con lo stesso nome del file zip, senza estensione

        with zipfile.ZipFile(cos_zip, 'r') as zipper:
            print("Unzipping CosNEU files...")
            zipper.extractall(final_dir_cos)

        cos_data = os.listdir(final_dir_cos)
        for i in cos_data:
            if ".tif" in i:
                i = final_dir_cos + "\\" + i
...
file_name_matrixEast = folder + "\\" + "EW_.tif"
...
metadata_file_path = folder + "\\" + "EW.metadata"
...
# 6) Set the name of the file and save the file
file_name_matrixUp = folder + "\\" + "UP_.tif"
